...
import fr.sparna.rdf.sesame.toolkit.skos.GetConceptsInSchemeHelper;
...
public class ConceptListSkosReader {
	
	public static final List<String> EXTENDED_SKOS_PROPERTIES = Arrays.asList(new String[] {
			SKOS.NOTATION,
			SKOS.ALT_LABEL,
			SKOS.BROADER,
			SKOS.NARROWER,
	...
	protected List<String> skosPropertiesToRead = EXTENDED_SKOS_PROPERTIES;

	public ConceptListSkosReader(Repository r) {
		super();
		this.repository = r;
	}
	...
	throws SPARQLExecutionException {
		...
		if(addSections) {
			...
			for (EntrySpec anEntry : entries) {
				...
				String entrySectionTitle = StringUtil.withoutAccents(anEntry.prefLabel).toUpperCase().substring(0, 1);
	...
	throws SPARQLExecutionException {
		...
		String entryId = Integer.toString((anEntry.conceptURI+anEntry.prefLabel).hashCode());
		e = SchemaFactory.createEntry(entryId, anEntry.conceptURI, anEntry.prefLabel);
